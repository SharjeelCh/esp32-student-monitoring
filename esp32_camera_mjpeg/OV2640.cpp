...
#define TAG "OV2640"
#include "sensor.h" // Ensure this is included for sensor control

void OV2640::setVerticalFlip(bool enable)
{
    sensor_t *s = esp_camera_sensor_get();
    if (s) {
        s->set_vflip(s, enable ? 1 : 0); // 1 for enable, 0 for disable
    }
}

void OV2640::setHorizontalFlip(bool enable)
{
    sensor_t *s = esp_camera_sensor_get();
    if (s) {
        s->set_hmirror(s, enable ? 1 : 0); // 1 for enable, 0 for disable
    }
}
